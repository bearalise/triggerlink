...
| `event` | The triggering event, echoed verbatim (`data` is arbitrary JSON, `ts` is RFC3339). For a batch-triggered run this is the **first** event of the batch |
| `events` | **Batch triggers only** (FR-4.7, function configured with `batch`): the whole batch in arrival order, each element shaped like `event`. Absent for ordinary triggers. `event` is always `events[0]` when present, so an SDK that only reads `event` still receives a well-formed event |
